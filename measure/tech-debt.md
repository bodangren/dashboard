...
| 2026-04-25 | search-filtering_20260425 | Search UI: Filter panel hidden by default until user toggles it | Low | Resolved | Phase 3 work complete; filter toggle shows panel on demand |
| 2026-05-02 | flaky-test-fix_20260502 | FLaky-01: TestHub_Run_PanicRecoveryContainsMessage fails non-deterministically | Medium | Resolved | Replaced done channel with sync.WaitGroup to avoid logBuf race. Added sync.Mutex to LogWatcher to protect inotifyFd writes during initInotify vs Stop. |
| 2026-05-02 | flaky-test-fix_20260502 | FLaky-02: TestWatcherManager_StartStop/StopAll race on inotifyFd | Medium | Resolved | LogWatcher.Stop() now acquires mu before closing inotifyFd; initInotify acquires mu when setting fd. Prevents concurrent close/write of inotifyFd. |
...
| 2026-05-05 | platform-git-features_20260505 | FRONTEND-03: Branch/stash UI needs user testing | Low | Open | Branch dropdown and stash panel added to project cards; manual verification in browser pending |
| 2026-05-04 | dashboard_customization_20260503 | FRONTEND-02: No layout preferences system | Low | Resolved | Created preferences.js with density/sort preferences, settings panel with density and sort selectors, localStorage persistence, CSS density modes, sortProjects() with alphabetical/recent ordering. Section visibility toggles noted as N/A for main index page (separate pages handle agents/activity). |
| 2026-05-09 | review | DUP-01: CommitSearchQuery duplicated in api and search packages | Medium | Open | Cannot dedup without first breaking git->api dependency. Import cycle: git->api, api->search would create search->git->api->search. Current duplication is localized and non-functional. |
