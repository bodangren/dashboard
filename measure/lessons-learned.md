...
- (2026-05-07, project_tagging_workspaces_20260507) TagManager with localStorage versioning works for client-side tag storage. getAllTags() sorted unique across repos enables filter UI. Directory grouping with collapsible sections using CSS toggle + hidden class works without JS state management.
- (2026-05-07, advanced_search_indexing_20260507) Search indexer initialized in main.go before API registration; BuildFromRepos blocks startup but is fast (~100ms for 22 repos). Background goroutine with time.Ticker updates index incrementally.
- (2026-05-08, cross_repo_commit_search_20260508) Avoid import cycles: define CommitSearchQuery in api package rather than search. Conversion happens in main.go adapter layer. Frontend: author dropdown populated from knownAuthors set gathered at load time.