...
- **Root Cause (multiple sub-issues):**
  ...
  2. **Merge conflict:** 1 repo (conductor) has unmerged files. `git pull --ff-only` fails with "unresolved conflict."
...
- **Root Cause:** Agent `log_path` values from the crontab are **relative paths** like `conductor/output.log`. The `ReadLogFile` function in `internal/agents/logreader.go` opens the path directly with `os.Open(path)`. Since the Go server's working directory is `/home/daniel-bo/Desktop/dashboard`, it tries to open `/home/daniel-bo/Desktop/dashboard/conductor/output.log` instead of `/home/daniel-bo/Desktop/<agent-directory>/conductor/output.log`. The file doesn't exist at that path, so `os.Stat` returns "not exist" and the handler returns `{exists: false}`.
...
- **Evidence:** `curl http://localhost:8080/api/agents/3/log` returns `{"exists":false,...}` despite `/home/daniel-bo/Desktop/advantage-games/conductor/output.log` existing.
...
- Resolving the merge conflict on the conductor repo
