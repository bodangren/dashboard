...
| 2026-04-24 | agent-log-streaming_20260424 | ARCH-11: inotify unavailable on non-Linux systems | Low | Open | LogWatcher falls back to 500ms polling when inotify_init fails. Cross-platform file watching (kqueue/FSEvents) could replace this. |
| 2026-04-25 | review | RACE-01: AgentStateMap has no mutex — concurrent goroutine writes from runAgentAsync vs HTTP handler reads | High | Open | types.go:104-117. Needs sync.RWMutex on Get/Set/Clear. |
| 2026-04-25 | review | LEAK-01: LogStreamHandler.ServeHTTP goroutine leaks — `<-make(chan struct{})` never detects client disconnect | High | Open | log_stream_handler.go:46. Need conn.Read goroutine to detect close, then signal exit. |
| 2026-04-25 | review | LEAK-02: Hub broadcast removes conns without calling conn.Close() — WebSocket FD leak | High | Open | hub.go:84-114. Add conn.Close() when removing from clients/subscriptions. |
| 2026-04-25 | review | SAFE-01: triggerAgent runs exec.Command with user-supplied binary path — no validation | Critical | Open | agent_handlers.go:257-321. BinaryPath/Harness need whitelist validation. |
| 2026-04-25 | review | SAFE-02: agent_executor.go nil dereference on cmd.ProcessState after unexpected kill | Critical | Open | agent_executor.go:49. Check ProcessState != nil before calling ExitCode(). |
| 2026-04-25 | review | LEAK-03: Hub.Stop() closes channels causing run() to spin on zero values | Medium | Open | hub.go:134-145. run() should check channel closure or use done channel pattern. |
